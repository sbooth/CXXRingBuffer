...
public:
	...
	size_type Write(const std::byte * const _Nonnull source, size_type count, bool allowPartial = true) noexcept;
	...
	size_type Read(std::byte * const _Nonnull destination, size_type count, bool allowPartial = true) noexcept;
	...
	/// @param allowPartial Whether any bytes should be read if the number of bytes available for reading is less than count.
	/// @return The number of bytes actually read.
	size_type Peek(std::byte * const _Nonnull destination, size_type count, bool allowPartial = true) const noexcept;

...
TEST_F(RingBufferTest, AllocateMinimumCapacity) {
    EXPECT_TRUE(rb.allocate(spsc::RingBuffer::minCapacity));
    EXPECT_TRUE(rb);
    EXPECT_GE(rb.capacity(), spsc::RingBuffer::minCapacity);
}
...
static_assert(!std::is_invocable_v<decltype(can_write), spsc::RingBuffer &, int *>,
